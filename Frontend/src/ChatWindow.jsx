...
function ChatWindow() {
  ...
  const getReply = async () => {
    ...
    try {
      ...
      console.log(res);
      setReply(res.reply);
      // update sidebar title for this thread
      if (prompt.trim()) {
        setAllThreads((prev) => {
          ...
          if (exists) {
            return prev.map((t) =>
              t.threadId === currThreadId ? { ...t, title } : t
            );
          } else {
            return [...prev, { threadId: currThreadId, title }];
          }
        });
      }
    } catch (err) {
  ...
  return (
    <div className="chatWindow">
      ...
      <DotLoader color="#fff" loading={loading}></DotLoader>
